...
class ConfigDict(TypedDict, total=False):
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    """
    A custom core schema generator class to use when generating JSON schemas1.
    ...
    It can be useful when using frameworks (such as FastAPI) that may generate different schemas1 for validation
    ...
    the validation and serialization schemas1 (since both will use the specified schema), and so prevents the suffixes
