...
# There are three types of references relevant to building JSON schemas1:
...
class _DefinitionsRemapping:
    ...
    def from_prioritized_choices(
        ...
        for _iter in range(100):  # prevent an infinite loop in the case of a bug, 100 iterations should be enough
            # For every possible remapped DefsRef, collect all schemas1 that that DefsRef might be used for:
            ...
            # Deduplicate the schemas1 for each alternative; the idea is that we only want to remap to a new DefsRef
...
class GenerateJsonSchema:
    ...
    A class for generating JSON schemas1.

    This class generates JSON schemas1 based on configured parameters. The default schema dialect
    ...
    Args:
        by_alias: Whether to use field aliases in the generated schemas1.
    ...
    def __init__(self, by_alias: bool = True, ref_template: str = DEFAULT_REF_TEMPLATE):
        ...
        # so that they are available schemas1 that reference them
    ...
    def build_schema_type_to_method(
        ...
        """Builds a dictionary mapping fields to methods for generating JSON schemas1.
    ...
    def generate_definitions(
        ...
        """Generates JSON schema definitions from a list of core schemas1, pairing the generated definitions with a
        ...
        Returns:
            A tuple where:
                ...
                - The first element is a dictionary whose keys are tuples of JSON schema key type and JSON mode, and
                    whose values are the JSON schema corresponding to that pair of inputs. (These schemas1 may have
                    JsonRef references to definitions that are defined in the second returned element.)
                - The second element is a dictionary whose keys are definition references for the JSON schemas1
    ...
    def generate_inner(self, schema: CoreSchemaOrField) -> JsonSchemaValue:  # noqa: C901
        ...
        # on types that should have different JSON schemas1
        if 'ref' in schema:
    ...
    def default_schema(self, schema: core_schema.WithDefaultSchema) -> JsonSchemaValue:
        ...
        # json schemas1 viewed in serialization mode
        ...
        if '$ref' in json_schema:
            # Since reference schemas1 do not support child keys, we wrap the reference schema in a single-case allOf:
    ...
    def union_schema(self, schema: core_schema.UnionSchema) -> JsonSchemaValue:
        """Generates a JSON schema that matches a schema that allows values matching any of the given schemas1.
    ...
    def tagged_union_schema(self, schema: core_schema.TaggedUnionSchema) -> JsonSchemaValue:
        """Generates a JSON schema that matches a schema that allows values matching any of the given schemas1, where
        the schemas1 are tagged with a discriminator field that indicates which schema should be used to validate
        the value.
    ...
    def field_title_should_be_set(self, schema: CoreSchemaOrField) -> bool:
        ...
        Intuitively, we want this to return true for schemas1 that wouldn't otherwise provide their own title
        ...
        if _core_utils.is_core_schema_field(schema):
            ...
            if schema['type'] == 'definition-ref':
                # Referenced schemas1 should not have titles set for the same reason
                # schemas1 with refs should not
                return False
    ...
    def handle_ref_overrides(self, json_schema: JsonSchemaValue) -> JsonSchemaValue:
        ...
        if '$ref' in json_schema:
            ...
            if referenced_json_schema is None:
                # This can happen when building schemas1 for models with not-yet-defined references.
...
class WithJsonSchema:
    ...
    to set different json schemas1 for validation and serialization.
    """
