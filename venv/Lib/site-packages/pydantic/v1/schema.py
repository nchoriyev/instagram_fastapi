...
def schema(
    ...
    *,
    ...
    :param by_alias: generate the schemas1 using the aliases defined, if any
    ...
    :param ref_prefix: the JSON Pointer prefix for schema references with ``$ref``, if None, will be set to the
      default of ``#/definitions/``. Update it if you want the schemas1 to reference the definitions somewhere
      else, e.g. for OpenAPI use ``#/components/schemas1/``. The resulting generated schemas1 will still be at the
    ...
    :param ref_template: Use a ``string.format()`` template for ``$ref`` instead of a prefix. This can be useful
      ...
      a sibling json file in a ``/schemas1`` directory use ``"/schemas1/${model}.json#"``.
...
def model_schema(
    ...
    :param ref_template: Use a ``string.format()`` template for ``$ref`` instead of a prefix. This can be useful for
      ...
      sibling json file in a ``/schemas1`` directory use ``"/schemas1/${model}.json#"``.
...
def field_schema(
    ...
    *,
    ...
    Also return a dictionary of definitions with models as keys and their schemas1 as values. If the passed field
    ...
    :param ref_prefix: the JSON Pointer prefix to use for references to other schemas1, if None, the default of
